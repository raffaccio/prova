...
document.addEventListener("DOMContentLoaded", function () {
    ...
    window.addEventListener("scroll", function () {
        ...
        if (scrollTop > lastScrollTop && scrollTop > upperHeaderHeight) {
            ...
            upperHeader.classList.add("no-transition"); // Temporarily disable transition
            document.body.classList.remove("upper-header-hidden");

            // Remove the no-transition class after a short delay
            setTimeout(() => {
                upperHeader.classList.remove("no-transition");
            }, 10); // Minimum delay to ensure the transition is disabled
        }
